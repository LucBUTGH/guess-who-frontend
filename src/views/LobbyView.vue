...
function handleEvent(event: GameEvent) {
  if (event.type === "GAME_START") {
    localStorage.setItem(
      `char-${waitingCode.value}`,
      JSON.stringify(event.character),
    );
    router.push(`/game/${waitingCode.value}`);
  }
}
...
<template>
  <div class="min-h-screen bg-[#07080F]">
    ...
    >
      <div
        class="absolute top-0 left-1/2 -translate-x-1/2 w-200 h-[30rem] rounded-full bg-amber-500/[0.04] blur-3xl"
      ></div>
    </div>

    <header
      class="border-b border-white/[0.06] bg-[#07080F]/80 backdrop-blur sticky top-0 z-10"
    >
      <div
      ...
      >
        <div class="flex items-center gap-3">
          <router-link to="/" class="group flex items-baseline gap-2">
            <span
              class="text-base font-black text-zinc-100 tracking-tight group-hover:text-amber-400 transition-colors"
              >Guess Who</span
            >
          </router-link>
          <div class="w-px h-4 bg-white/[0.08]"></div>
          <span class="text-zinc-500 text-xs uppercase tracking-widest"
            >Lobby</span
          >
        </div>
        <div class="flex items-center gap-4">
          <div
            v-if="stats"
            class="hidden sm:flex items-center gap-3 text-xs font-mono"
          >
    ...
    <main class="max-w-3xl mx-auto px-6 py-14">
      ...
      >
        <p
          class="text-amber-400 text-[10px] uppercase tracking-[0.3em] font-medium mb-12"
        >
        ...
        <div class="relative mb-10">
          <div
            class="absolute inset-0 rounded-3xl bg-amber-400/[0.06] blur-3xl scale-150 pointer-events-none"
          ></div>
          <div
            class="relative bg-[#0D1120] border border-amber-400/20 rounded-2xl px-8 py-6 sm:px-16 sm:py-8"
          >
            <p
              class="text-[10px] text-zinc-600 uppercase tracking-[0.3em] mb-4"
            >
              Code de la partie
            </p>
            <p
              class="text-5xl sm:text-7xl font-black tracking-[0.15em] text-amber-400 font-mono select-all"
            >
              {{ waitingCode }}
            </p>
          </div>
        ...
        <div class="flex items-center gap-2 mt-14 text-zinc-700 text-xs">
          <div
            class="w-1.5 h-1.5 rounded-full bg-amber-400/60 animate-pulse"
          ></div>
      ...
      <template v-else>
        <div class="mb-12">
          <p
            class="text-amber-400 text-[10px] uppercase tracking-[0.3em] font-medium mb-3"
          >
            Prêt ?
          </p>
          <h2
            class="text-4xl sm:text-5xl font-black text-zinc-100 tracking-tight"
          >
            Nouvelle partie
          </h2>
        </div>
        ...
        <div class="grid gap-6 sm:grid-cols-2">
          <div
            class="bg-[#0D1120] border border-white/[0.06] rounded-2xl p-8 flex flex-col gap-6"
          >
            <div>
              <h3 class="text-lg font-black text-zinc-100 mb-1">Créer</h3>
              <p class="text-xs text-zinc-600 leading-relaxed">
                Lance une nouvelle partie et partage le code avec ton adversaire
              </p>
            </div>
            <button
              :disabled="loading"
          ...
          >
            <div>
              ...
              <p class="text-xs text-zinc-600 leading-relaxed">
                Entre le code que ton adversaire t'a communiqué
              </p>
            </div>
        ...
        <p v-if="error" class="mt-6 text-xs text-rose-400 text-center">
          {{ error }}
        </p>
      </template>
    </main>
